...
void UnaryFunction::applyWithArgs(Expr& target, ListNode* args,
Environment* rho)
{
    if (!fun)
...
void BinaryFunction::applyWithArgs(Expr& target, ListNode* args,
Environment* rho)
{
    if (!fun)
...
UserFunction::UserFunction(ListNode* anames, Expression* bod,
Environment* ctx)
{
    argNames = anames;
    body = bod;
